...
\begin{action}{Remove port opening}
\cmd{\windows}{netsh firewall delete portopening protocol =TCP port=3389 name=RDP}
\end{action}

\begin{action}{Open Port Range}
\cmd{\windows}{for /l \%x in (1000,1,1100) do netsh firewall add portopening protocol=TCP port=\%x name=HighRange mode=ENABLE}
\end{action}

\subsubsection{Win Vista and Later}
\begin{action}{Enable Windows firewall in block mode}
    \cmd{\windows}{netsh advfirewall set allprofiles state on}
    \cmd{\windows}{netsh advfirewall set allprofiles firewallpolicy "blockinboundalways,allowoutbound"}
\end{action}

\begin{action}{Close ports (outbound)}
\cmd{\windows}{netsh advfirewall firewall add rule name="BlockAIM" protocol=TCP\\
dir=out remoteport=4099 action=block}
\end{action}

\begin{action}{Block Port Range (inbound)}
\cmd{\windows}{netsh advfirewall firewall add rule name="BlockHigh" protocol=TCP dir=in port=10-100 action=block}
\end{action}

\begin{action}{Open ports (inbound)}
\cmd{\windows}{netsh advfirewall firewall add rule name="Allow HTTP" protocol=TCP\\
dir=in localport=80 action=allow}
\end{action}

\subsection*{Network}

\begin{action}{Display listening TCP/UDP ports}
\cmd{\windows}{netstat -abon | select-string -Context 1 LISTENING} \emph{(PowerShell Only)}
...
\cmd{\windows} {needs to be filled in}
\end{action}
...
\begin{action}{Open Control Panel}
\cmd{\windows}{control}
\end{action}

\begin{action}{Open Services}
\cmd{\windows}{services.msc}
\end{action}

\begin{action}{Open Remote Desktop}
\cmd{\windows}{mstsc}
\end{action}

\begin{action}{Open AD Users and Computers}
\cmd{\windows}{dsa.msc}
\end{action}

\begin{action}{Open DNS Manager}
\cmd{\windows}{dnsmgmt.msc}
\end{action}

\begin{action}{Open Group Policy Editor}
\cmd{\windows}{gpedit.msc}
\end{action}
